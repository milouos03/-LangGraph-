...
def _format_workflow_error(exc: Exception) -> str:
    ...
    if isinstance(exc, KeyError) and exc.args == ("request",):
        return (
            "模型服务调用失败：DashScope/百炼返回了错误响应，但当前 LangChain 适配器把真实错误包装成了 'request'。"
            "请检查 DASHSCOPE_API_KEY 是否有效、模型额度是否用完，或是否开启了仅使用免费额度模式。"
        )
    ...
    async def stream_events(
        ...
        def worker() -> None:
            try:
                ...
                emit({"type": "error", "message": _format_workflow_error(exc)})
            finally:
                emit({"type": "__done__"})
