...
#intent节点的路由函数，根据当前状态的intent值决定下一步执行的节点
...
#should_continue_research函数根据当前状态的iteration和max_iterations值，以及是否需要更多研究，决定下一步执行的节点
...
def build_app(agents, checkpointer):
    ...
    # 这个函数返回一个编译后的工作流对象，它会使用 checkpointer 来保存和恢复状态。
