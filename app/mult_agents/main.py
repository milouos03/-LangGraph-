...
# 这个函数从 Agent 的消息列表中收集工具调用信息 存入tools和tool_outputs两个列表中
...
# 这个函数会把跨会话记忆拼接到用户提示词后面，形成完整的输入给 Agent
...
#定义plan节点 用记忆上下文获取state里的query和plan，构成humanmessage
#然后调用agent.invoke方法执行plan节点的逻辑，获取结果并记录日志，最后返回更新后的state
...
def web_search_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    # 记录 web_search 节点开始执行。
    # colorize("[web_search]", "cyan") 会把节点名显示成青色。
    # colorize(agent_name, "magenta") 会把 Agent 名显示成洋红色。
    logger.info("%s 开始 | agent=%s", colorize("[web_search]", "cyan"), colorize(agent_name, "magenta"))

    # 打印当前节点的输入数据，方便调试。
    # 这里把 plan 和 query 记录下来：
    # plan 是前面 plan 节点生成的研究计划；
    # query 是用户最初提出的问题。
    log_inputs("web_search", agent_name, {"plan": state["plan"], "query": state["query"]})

    # 创建一条 HumanMessage，作为发给当前 Agent 的用户消息。
    # with_memory_context(...) 会把跨会话记忆拼接到提示词后面。
    # 提示词里包含两部分：
    # 1. 计划：state["plan"]
    # 2. 问题：state["query"]
    ...
    # 调用当前节点绑定的 Agent。
    # state["messages"] 是之前节点累计的历史消息；
    # [human] 是当前新构造的用户消息；
    # 两者相加后一起传给 Agent，让它能看到上下文。
    result = agent.invoke({"messages": state["messages"] + [human]})

    # 从 Agent 返回的消息列表中取最后一条。
    # 通常最后一条就是 Agent 最新生成的回答，也就是 web_search 节点的输出。
    last_message = result["messages"][-1]

    # 从所有消息里收集工具调用信息。
    # tools 是工具名称列表；
    # tool_outputs 是工具输出内容列表。
    tools, tool_outputs = collect_tool_calls(result["messages"])

    # 打印当前节点调用过哪些工具。
    # 如果 tools 不为空，就用 ", ".join(tools) 拼成字符串；
    # 如果没有工具调用，就显示 "无"。
    logger.info("%s 工具: %s", colorize("[web_search]", "green"), ", ".join(tools) if tools else "无")

    # 最多打印前 5 条工具输出，避免日志过长。
    for item in tool_outputs[:5]:
        # 每条工具输出最多打印前 400 个字符，避免刷屏。
        logger.info("%s 工具输出: %s", colorize("[web_search]", "green"), item[:400])

    # 打印说明：这个节点调用了 LLM。
    # “思考: 不可见”表示不会输出模型内部推理过程。
    logger.info("%s LLM调用: 是 | 思考: 不可见", colorize("[web_search]", "yellow"))

    # 记录 web_search 节点的最终输出预览。
    # emit 内部会把换行替换为空格，并截断过长文本。
    emit("web_search", last_message.content)

    # 返回一个字典，告诉 LangGraph 要更新哪些 state 字段。
    return {
        # 把 Agent 生成的网络搜索结果保存到 state["web_search"]。
        "web_search": last_message.content,
        # 把当前用户消息和 Agent 回复追加到 state["messages"]。
        # 因为 ResearchState 里 messages 使用了 operator.add，所以这里是追加，不是覆盖。
        "messages": [human, last_message],
    }
...
def build_agents(model: str, api_key: str, config: AppConfig) -> AgentBundle:
    if config.enable_milvus:
        ...
        logger.info("%s Milvus/RAG disabled by config", colorize("[rag]", "yellow"))
...
#初始化配置 获取用户问题 调用run_query方法执行研究任务，并返回最终结果
def main():
