...
#在本地知识库对query进行检索，返回前k条最相关的记录，并将其整理为字典列表，每条记录包含source_id、title、url、snippet、domain、source_type和published_at等信息。
...
#向Bocha API发送请求进行网页搜索
# 返回前count条最相关的网页记录，并将其整理为record字典列表
# 每条记录包含source_id、title、url、snippet、domain、source_type和published_at等信息。
...
#这个工具函数用于计算简单的算术表达式。
# 它接收一个字符串形式的表达式作为输入，解析该表达式，并返回计算结果。
# 支持的运算符包括加法、减法、乘法、除法、幂运算和取模运算。
@tool
