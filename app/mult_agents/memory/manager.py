...
#这个函数是用来为特定的用户和线程构建上下文信息的
# 包括用户画像、最近的对话消息、相关的记忆条目、最近的任务记录以及对话摘要。它会根据提供的查询参数，从长短期记忆中检索相关信息，并将这些信息组织成一个字典返回，以便在生成个性化提示时使用。
#主要步骤：1. 获取用户画像（如果长时记忆范围是“用户”）。
# 2. 获取最近的对话消息。
# 3. 如果提供了查询参数，则从长时记忆中搜索相关的记忆条目。
# 4. 获取最近的任务记录。
# 5. 获取对话摘要。
# 6. 将所有信息组织成一个字典并返回。 
...
# 这个函数是用来为特定的用户和线程构建个性化的提示上下文的
# 它会调用 get_context_for_agent 方法获取上下文信息，然后将这些信息格式
#1. 获取上下文信息，包括用户画像、最近的对话消息、相关的记忆条目、最近的任务记录以及对话摘要。
#2. 格式化记忆条目，将其转换为适合在提示中使用的文本格式。
#3. 将用户画像、最近的对话消息、对话摘要和记忆条目组织成不同的部分，并将它们组合成一个完整的提示上下文字符串。
#4. 记录注入信息，包括租户ID、用户ID、线程ID、查询内容、是否注入了用户画像、摘要字符数、记忆
    def build_personalized_prompt_context(
        ...
        # 记录每个记忆条目的来源和摘要信息，以便在日志中进行跟踪和分析。
        ...
        # 记录最后一次的注入信息，包括租户ID、用户ID、线程ID、查询内容、是否注入了用户画像、摘要字符数、记忆条目数量、来源数量、注入的字符数以及注入的预览信息。
        self._last_trace = {
...
#这个函数主要用于在对话中保存用户的查询和AI的回答，
# 并根据用户的查询内容判断是否需要提取长期记忆的事实和偏好。
# 如果用户的查询中包含了记忆标记词，
# 则会尝试从消息中提取长期记忆的事实和偏好，
# 并保存到长期记忆中。
# 同时，它还会将用户的查询和AI的回答保存到短期记忆中，以便在后续的对话中使用。
    def persist_turn(
        self,
        tenant_id: str,
        user_id: str,
        ...
        #获取短期记忆的消息，并将用户的查询和AI的回答保存到短期记忆中，以便在后续的对话中使用。
        ...
        #加入长期记忆的提取逻辑，如果用户的查询中包含了记忆标记词，则会尝试从消息中提取长期记忆的事实和偏好，并保存到长期记忆中。
        ...
        #用于判断提取的事实或偏好是否有效的函数，避免将无效或不相关的信息保存到长期记忆中。
        ...
        #根据用户的查询内容，判断是否允许提取第二人称的记忆信息。如果用户的查询中包含了与第二人称相关的标记词，则允许提取第二人称的记忆信息。
        allow_second_person = any(token in lower_query for token in ["以后你", "你叫做", "你的偏好", "回答偏好", "你要"])
        #根据提取的事实和偏好，过滤掉无效的候选项，并将有效的事实和偏好保存到长期记忆中。如果没有提取到有效的事实或偏好，但用户的查询本身是一个有效的候选项，则将用户的查询作为偏好保存到长期记忆中。
        ...
        #如果应该提取长期记忆，但没有提取到有效的事实或偏好，并且用户的查询是一个有效的候选项，则将用户的查询作为偏好保存到长期记忆中。
        ...
        #判断偏好是否存在
        #如果存在偏好，根据长期记忆的范围（用户或线程），将偏好保存到用户画像或作为事实保存到长期记忆中。
        if preferences:
            if self.long_term_scope == "user":
