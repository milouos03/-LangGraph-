...
#logger的输出函数，打印节点的输出内容，超过400字符的内容会被截断
...
#获取messages中所有的工具调用信息，包括工具名称和输出内容
# 并返回两个列表：tools和tool_outputs 用于存储工具名称和工具输出内容
...
#将state中的memory_context与用户的输入提示拼接起来，形成一个完整的输入内容
...
#根据 关键词 判断用户的查询意图是直接回答还是多智能体研究
def detect_intent(query: str) -> str:
    normalized_query = query.strip()  #标准化查询
    force_multiagent_keywords = [  #强制多智能体关键词
        "调查",
        "调研",
        "来源",
        "证据",
...
#获取state中最后一条消息的内容
#如果是字符串则直接返回，如果是列表则将每个元素的text字段拼接成字符串返回
...
#提取文本中的 JSON 块，去掉代码块标记和多余的空白字符 返回一个字符串
...
#调用JSON智能体 注入了state和prompt
# 返回解析后的JSON字典、原始内容和消息列表
...
#获取query中的主要实体，优先返回英文实体，如果没有则返回中文实体
# 如果都没有则返回空字符串
...
#提取query中的关键词，格式化之后猜测主要query实体，返回一个列表
#目的是扩充多条检索查询，增加检索覆盖面
...
#判断候选检索词是否与用户的原始查询有足够的相关性，返回布尔值
...
#根据大纲、子问题和研究问题生成搜索计划，返回一个列表
#用于指导后续的检索节点，确保检索覆盖用户关心的主题
...
#根据 source_preference 筛选出符合条件的检索查询，返回一个列表
#用于指导后续的检索节点，确保检索覆盖用户关心的主题
...
def _is_official_domain(domain: str) -> bool:
    value = domain.lower()
    official = ["github", "gitlab", "stackoverflow", "medium", "arxiv"]
    return any(item in value for item in official)

#筛选网页记录
#返回两个列表：一个是保留的记录，一个是统计信息
#统计的信息包括原始记录数、保留记录数、丢弃的无关记录数、丢弃的域名记录数、丢弃的空记录数
...
#筛选本地知识库记录
#返回两个列表：一个是保留的记录，一个是统计信息
#统计的信息包括原始记录数、保留记录数、丢弃的无用记录数、丢弃的缺失文档记录数、丢弃的空记录数
...
#将记录列表转化json字符串，最多保留40条记录，每条记录包含 source_id、title、url、doc_id、snippet 和 source_type 字段
...
#意图识别节，判断是直接回答还是多轮检索
#返回要更新的 state，包含 intent 字段，值为 "direct" 或 "multiagent"
def intent_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    # 如果 LLM 输出的 route 不合法，则回退到规则引擎的结果
...
#直接回答节，调用 LLM 直接生成答案
#返回要更新的 state，包含 intent 字段，值为 "direct"，以及 final、draft、analysis_summary 等字段
...
def plan_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    # 通过 LLM 生成调研规划，包括大纲、问题拆解、研究问题、搜索计划和预算
    #payload是LLM输出的JSON数据 包括大纲、问题拆解、研究问题、搜索计划和预算
    #content是原始文本
    #messages是与LLM交互的消息列表
    payload, content, messages = _invoke_json_agent(
        state,
    ...
    #search_plan是LLM输出的搜索计划列表 包括每个搜索步骤的section_id、query、source_preference和reason
...
#这个函数主要是用于执行网页搜索节点的操作。它会根据当前的研究状态（ResearchState）和指定的代理（agent）来构建查询，执行网页搜索，并处理返回的原始记录。具体步骤如下：
#1. 构建查询：根据当前的研究状态，调用_build_queries函数生成一组查询，这些查询将用于网页搜索。
#2. 执行搜索：对于每个查询，调用bocha_web_search_records函数执行网页搜索，并获取返回的记录。
#3. 处理记录：对返回的记录进行去重和过滤，确保只保留有用的记录。同时，将每条记录与其对应的查询和章节ID关联起来。
#4. 调用LLM整理证据：将处理后的原始记录传递给LLM（大语言模型），请求其整理成结构化的JSON格式证据。
#5. 更新状态：将整理后的证据、检索统计信息和查询跟踪信息更新到研究状态中。如果没有可用的网页证据，则会记录警告并跳过网页上下文注入。
...
# 这个函数主要是用于执行本地知识库检索节点的操作。它会根据当前的研究状态（ResearchState）和指定的代理（agent）来构建查询，执行本地知识库检索，并处理返回的原始记录。具体步骤如下：
# 1. 构建查询：根据当前的研究状态，调用_build_queries函数生成一组查询，这些查询将用于本地知识库检索。
# 2. 执行检索：对于每个查询，调用search_knowledge_base_records函数执行本地知识库检索，并获取返回的记录。
# 3. 处理记录：对返回的记录进行去重和过滤，确保只保留有用的记录。同时，将每条记录与其对应的查询和章节ID关联起来。
# 4. 调用LLM整理证据：将处理后的原始记录传递给LLM（大语言模型   ），请求其整理成结构化的JSON格式证据。
# 5. 更新状态：将整理后的证据、检索统计信息和查询跟踪信息更新到研究状态中。如果没有可用的本地知识库证据，则
...
# 这个函数主要是用于执行深度分析节点的操作。它会根据当前的研究状态（ResearchState）和指定的代理（agent）来对已有的网页证据和本地知识库证据进行评分、去重和冲突审计，并生成最终的证据池。具体步骤如下：
# 1. 检查证据：如果当前状态中没有网页证据和本地证据，则记录等待检索结果的日志，并返回空字典。
# 2. 调用LLM进行审计：如果有证据，则调用LLM（大语言模型）对网页证据和本地证据进行评分、去重和冲突审计，并请求其输出结构化的JSON格式结果。
# 3. 处理证据池：从LLM返回的证据池中筛选出合法的证据，并补充原始证据中可能丢失的字段（如标题、URL等）。
# 4. 生成审计标记和来源索引：根据处理后的证据池生成审计标记和来源索引，确保每个来源都有唯一的标识。
# 5. 返回更新后的状态：将深度分析的结果，包括证据池、审计标记和来源索引，更新到研究状态中，并返回给调用者。
...
# 这个函数主要是用于执行分析节点的操作。它会根据当前的研究状态（ResearchState）和指定的代理（agent）来对已有的证据池进行分析，并生成结论映射、评估证据完备性等。具体步骤如下：
# 1. 调用LLM进行分析：将当前的研究状态、原问题、子问题、证据池和审计标记传递给LLM（大语言模型），请求其输出结构化的JSON格式分析结果。
# 2. 处理分析结果：从LLM返回的分析结果中提取结论映射、发现、是否需要更多研究、缺失的证据缺口和分析摘要。如果LLM返回的结果不合法，则使用回退的默认分析结果。
# 3. 返回更新后的状态：将分析的结果，包括分析摘要、发现、结论映射、是否需要更多研究、缺失的证据缺口和消息列表，更新到研究状态中，并返回给调用者。       
...
# 这个函数主要是用于执行反思节点的操作。它会根据当前的研究状态（ResearchState）和指定的代理（agent）来对已有的证据池进行反思，并生成新的补搜计划。具体步骤如下：
...
# 这个函数主要是用于执行撰写节点的操作。它会根据当前的研究状态（ResearchState）和指定的代理（agent）来撰写最终的 Markdown 研报。具体步骤如下：
